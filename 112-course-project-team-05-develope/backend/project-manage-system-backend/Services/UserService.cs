...
namespace project_manage_system_backend.Services
{
    public class UserService : BaseService
    {
        ...
        public UserInfoDto GetUser(string account)
        {
            ...
            Console.WriteLine("59 " + user.Bio);
            if (user == null)
                throw new Exception("User not found!");
            Console.WriteLine(user.Bio);
            return new UserInfoDto { id = user.Account, name = user.Name, avatarUrl = user.AvatarUrl, bio = user.Bio };
        }
        ...
        public void EditUserBio(string account, string bio)
        {
            User user = _dbContext.Users.Find(account);
            if (user != null)
            {
