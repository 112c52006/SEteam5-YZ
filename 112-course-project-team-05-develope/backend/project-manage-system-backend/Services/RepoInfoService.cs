...
namespace project_manage_system_backend.Services
{
    public class RepoInfoService : BaseService
    {
        ...
        //-- Get a Tree--//
        public async Task<ResponseTreeDto> RequestGetTree(int repoId, string oauthToken)
        {
            Console.WriteLine("RepoInfoService: RequestGetTree");
            Repo repo = GetRepoBy(repoId);
            IRepoInfo repoInfo = _repoInfoFactory.CreateRepoInfo(repo, _httpClient, oauthToken);
            return await repoInfo.RequestGetTree(repo);
        }

        //-- Get Language--//
        public async Task<String> RequestRepoLanguage(int repoId, string oauthToken)
        {
            Console.WriteLine("RepoInfoService: RequestRepoLanguage");
            Repo repo = GetRepoBy(repoId);
            IRepoInfo repoInfo = _repoInfoFactory.CreateRepoInfo(repo, _httpClient, oauthToken);
            return await repoInfo.RequestRepoLanguage(repo);
        }
