...
namespace project_manage_system_backend.Controllers
{
    ...
    public class RepoInfoController : ControllerBase
    {
        ...
        //-- Get a Tree--//
        [Authorize]
        [HttpGet("gettree/{repoId}")]
        public async Task<IActionResult> GetTree(int repoId)
        {
            Console.WriteLine("Line54: RepoInfoController");
            string oauthToken = User.Claims.FirstOrDefault(c => c.Type.Equals("oauth")).Value;
            return Ok(await _repoInfoService.RequestGetTree(repoId, oauthToken));
        }

        //-- Get Repolanguage--//
        [Authorize]
        [HttpGet("language/{repoId}")]
        public async Task<IActionResult> GetRepoLanguage(int repoId)
        {
            Console.WriteLine("Line54: RepoInfoController : GetRepoLanguage");
            string oauthToken = User.Claims.FirstOrDefault(c => c.Type.Equals("oauth")).Value;
            return Ok(await _repoInfoService.RequestRepoLanguage(repoId, oauthToken));
        }
