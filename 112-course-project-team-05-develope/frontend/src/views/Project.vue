...
<script lang="ts">
  import Vue from "vue";
  import { addProject, getProjects, deleteProject, getProjectImage } from "@/apis/projects.ts";
  ...
  export default Vue.extend({
    ...
    data() {
      return {
        ...
        snackBarColor: "",
        user: { type: Object, id: "", name: "", avatarUrl: "", bio: "" },
    ...
    async created() {
      ...
      await this.GetAllProjectImageUrls();
    },
    methods: {
      async updateProject() {
      ...
      async ProjectImageUrl(projectId: string){
      console.log(projectId);
      await getProjectImage(Number(projectId))
        .then(response => {
          const blob = response.data;
          const imageUrl = URL.createObjectURL(blob);
          this.$set(this.imageUrls, projectId, imageUrl);
        })
    },
    async GetAllProjectImageUrls() {
      await this.projects.forEach(async(item : any) => {
        await this.ProjectImageUrl(item.id);
        console.log(item.id);
      });
    }
    },
