<template>
  <v-container fluid fill-height class="container-background-color">
    <v-row class="d-flex justify-center">
      <!-- 左邊專案資訊 -->
      <v-col lg="2">
        <ProjectInfo :projectInfo="projectInfo" :projectId="projectId" :coverImageUrl="coverImageUrl"  @save="saveProjectInfo($event)" @loadImage="loadImage()" />
      </v-col>
      <!-- 左邊專案資訊 end -->
      <v-col lg="6">
        <v-row>
          ...
          <v-col>
            ...
            >
              ...
              <template v-slot:[`item.name`]="{ item }">
                ...
                <a
                  class="py-2"
                  @click="goToRepoInfo(item.id, item.type, item.boardId)"
                >
                  {{ item.name }}
                </a>
              </template>
...
<script lang="ts">
  ...
  import { editProject, getProject, getProjectMember,editProjectInfo ,getProjectImage} from "@/apis/projects";
  ...
  export default Vue.extend({
    ...
    methods: {
      ...
      //   );
      // },
      async goToRepoInfo(id: any, type: any, boardId: any) {
        if (type === "Jira") {
          this.$router.push({ name: "JiraInfo", params: { repoId: id } });
        } else if (type == "Trello") {
          this.$router.push({ name: "Trello", params: { boardId: boardId } });
        } else {
          this.$router.push({ name: "RepoInfo", params: { repoId: id } });
        }
      },
      async saveProjectInfo(newProjectInfo: string) {
      ...
      // 發送SnackBar給user看, 可傳入正確或錯誤以及要顯示的msg
      ...
      // 取得圖片
      async loadImage() {
      getProjectImage(Number(this.projectId))
        .then(response => {
