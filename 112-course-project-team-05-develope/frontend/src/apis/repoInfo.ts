...
//--Get a Tree--//
export const getTree = (repoId: number | null) => {
  return axios.get(`${host}/repoInfo/gettree/${repoId}`, {
    headers: {
      Authorization: `Bearer ${store.auth.getToken}`
    }
  });
...
//--Get Repo Language--//
export const getRepoLanguage = (repoId: number | null) => {
  return axios.get(`${host}/repoInfo/language/${repoId}`, {
    headers: {
      Authorization: `Bearer ${store.auth.getToken}`
    }
  });
